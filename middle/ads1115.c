...
extern float g_voltage[8];
 ...
 HAL_StatusTypeDef ADS1115_Init_C0(I2C_HandleTypeDef *handler, uint16_t setDataRate, uint16_t setPGA) {
     ...
     if (HAL_I2C_IsDeviceReady(&ADS1115_I2C_Handler, (uint16_t) (ADS1115_devAddress0 << 1), 5, ADS1115_TIMEOUT) == HAL_OK) {
         return HAL_OK;
     } else {
         return HAL_ERROR;
     }
 ...
 HAL_StatusTypeDef ADS1115_Init_C1(I2C_HandleTypeDef *handler, uint16_t setDataRate, uint16_t setPGA) {
    ...
    if (HAL_I2C_IsDeviceReady(&ADS1115_I2C_Handler, (uint16_t) (ADS1115_devAddress1 << 1), 5, ADS1115_TIMEOUT) == HAL_OK) {
        return HAL_OK;
    } else {
        return HAL_ERROR;
    }
...
HAL_StatusTypeDef ADS1115_readSingleEnded_C0(uint16_t muxPort, float *voltage) {
	...
	if(HAL_I2C_Mem_Write(&ADS1115_I2C_Handler, (uint16_t) (ADS1115_devAddress0 << 1), ADS1115_CONFIG_REG, 1, ADS1115_config, 2, ADS1115_TIMEOUT) == HAL_OK)
	{
		while(waiting) //Checking Data Ready
		{
			if(HAL_I2C_Mem_Read(&ADS1115_I2C_Handler, (uint16_t) ((ADS1115_devAddress0 << 1) | 0x1), ADS1115_CONFIG_REG, 1, ADS1115_config, 2, ADS1115_TIMEOUT) == HAL_OK)
				{
 ...
 HAL_StatusTypeDef ADS1115_readSingleEnded_C1(uint16_t muxPort, float *voltage) {
	...
	if(HAL_I2C_Mem_Write(&ADS1115_I2C_Handler, (uint16_t) (ADS1115_devAddress1 << 1), ADS1115_CONFIG_REG, 1, ADS1115_config, 2, ADS1115_TIMEOUT) == HAL_OK)
	{
		while(waiting) //Checking Data Ready
		{
			if(HAL_I2C_Mem_Read(&ADS1115_I2C_Handler, (uint16_t) ((ADS1115_devAddress1 << 1) | 0x1), ADS1115_CONFIG_REG, 1, ADS1115_config, 2, ADS1115_TIMEOUT) == HAL_OK)
				{
				if(ADS1115_config[0] & ADS1115_OS) waiting=0;
				}
			else return HAL_ERROR;
